...
namespace Lucene.Net.Util
{
    ...
    public class TestFixedBitSet : BaseDocIdSetTestCase<FixedBitSet>
    {
        ...
        internal virtual void DoRandomSets(int maxSize, int iter, int mode)
        {
            ...
            for (int i = 0; i < iter; i++)
            {
                ...
                if (b0 != null && b0.Length <= b.Length)
                {
                    ...
                    BitSet a_and = new BitSet(a.Length);
                    a_and = a_and.And_UnequalLengths(a0);
                    BitSet a_or = new BitSet(a.Length);
                    a_or = a_or.Or_UnequalLengths(a0);
                    BitSet a_xor = new BitSet(a.Length);
                    a_xor = a_xor.Xor_UnequalLengths(a0);
                    BitSet a_andn = new BitSet(a.Length);
                    a_andn.AndNot(a0);
        ...
        */
        ...
        private void CheckPrevSetBitSet(int[] a, int numBits)
        {
        ...
        {
            CheckPrevSetBitSet(new int[] { }, 0);
            CheckPrevSetBitSet(new int[] { 0 }, 1);
            CheckPrevSetBitSet(new int[] { 0, 2 }, 3);
        }

        private void CheckNextSetBitSet(int[] a, int numBits)
        {
            FixedBitSet obs = MakeFixedBitSet(a, numBits);
            BitSet bs = MakeBitSet(a);
            DoNextSetBit(bs, obs);
        ...
        public virtual void TestNextBitSet()
        {
            ...
            CheckNextSetBitSet(setBits, setBits.Length + Random.Next(10));

            CheckNextSetBitSet(new int[0], setBits.Length + Random.Next(10));
        }
